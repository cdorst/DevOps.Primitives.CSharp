...
namespace DevOps.Primitives.CSharp.EntityFramework.Services
{
        ...
        where TDbContext : CSharpDbContext
    {
        ...
        {
            CacheKey = record => $"{nameof(CSharp)}.{nameof(ClassDeclaration)}={record.AttributeListCollectionId}:{record.BaseListId}:{record.ConstraintClauseListId}:{record.ConstructorListId}:{record.DocumentationCommentListId}:{record.FieldListId}:{record.FinalizerId}:{record.IdentifierId}:{record.MethodListId}:{record.ModifierListId}:{record.NamespaceId}:{record.PropertyListId}:{record.TypeParameterListId}:{record.UsingDirectiveListId}";
